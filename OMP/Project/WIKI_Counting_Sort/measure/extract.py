...
def _extract(folderPath, plotColumns):
    ...
    filenames = [f for f in os.listdir('.') if f.endswith(
        ".csv") and re.match("SIZE-[0-9]+-NTH-[0-9]{2}-O[0-9]-?[0-9]*", f)]
    print(filenames)
...
if __name__ == "__main__":
    ...
    for folder in os.listdir(choosedFolder):
        ...
        header = {'values': ['Version', 'Threads', 'TimeInt',
                             'User', 'Sys', 'Real', 'Speedup', 'Efficiency']}
        cells = {'values': []}
        nt = -1
        ...
        table_filename = actualPath + "/psize-" + \
            size + "-" + str(opt) + "-table.csv"
        plot_filename = actualPath + "/speedup-" + \
            str(size) + "-" + str(opt) + ".jpg"
