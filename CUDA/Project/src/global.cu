...
/**
  @file global.cu
  @brief This is the file global.cu, which contains the main function and the other functions
 ...
 * @param n           number of array elements.
 */
__global__ void gpu_fullC(int *arrayA, int *arrayC, int n)
{
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= n)
    return;
  atomicAdd(&arrayC[arrayA[i]], 1);
}
...
float countingSortDEVICE(int *array_h, int n, int max, int blockSize)
{
  ...
  gpu_fullC<<<grid, block>>>(array_d, C_d, n);
...
{
  FILE *fp;
  char root_filename[] = "global_measure";
