...
/**
  @file texture.cu
  @brief This is the file texture.cu, which contains the main function and the other functions
...
texture<int, 1, cudaReadModeElementType> vet_text;
...
__global__ void gpu_fullC(int *arrayA, int *arrayC, int n, int lenC)
{
  ...
  if (input_idx < n)
    atomicAdd(&C_shared[tex1Dfetch(vet_text, input_idx)], 1);
  __syncthreads();
...
float countingSortDEVICE(int *array_h, int n, int max, int blockSize)
{
  ...
  cudaError_t errt = cudaBindTexture(NULL, vet_text, array_d, channA);
  if (errt != cudaSuccess)
  ...
  CUDA_CHECK(cudaUnbindTexture(vet_text));
...
{
  FILE *fp;
  char root_filename[] = "texture_measure";
